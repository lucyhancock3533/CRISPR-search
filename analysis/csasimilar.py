...
class SimilarityCalc:
    ...
    def __init__(self, cursor, evidenceLevel, similarityPercCutoff, dbPath, blastPath):
    ...
    def doSimilarityCalc(self):
        ...
        for genome in tqdm(genomeIds):
            ...
            if len(results) > 0:
                genomeResults.append((genome[1] + ' - ' + genome[2], [self.replaceIdForName(x) for x in results]))
